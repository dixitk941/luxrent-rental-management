      ...
      <section className="flex-1 min-w-0">
                  ...
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <div className="mt-auto">
                      ...
                      <Link
                        ...
                        className={product.status === 'booked' ? 'btn-secondary w-full text-xs py-2' : 'btn-primary w-full text-xs py-2'}
                      >
                        <span className="material-symbols-outlined" style={{fontSize:'16px'}}>calendar_month</span>
                        {product.status === 'booked' ? 'View Details' : 'Check Availability'}
                      </Link>
                    </div>
                  </div>
                </article>
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex justify-center items-center gap-1">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="p-2 rounded hover:bg-navy/5 text-slate transition-colors disabled:opacity-30">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
              <button
                key={p}
                onClick={() => setPage(p)}
                className={`w-9 h-9 rounded text-sm font-semibold transition-colors ${p === page ? 'bg-amber text-white' : 'hover:bg-navy/5 text-slate'}`}
              >
                {p}
              </button>
            ))}
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="p-2 rounded hover:bg-navy/5 text-slate transition-colors disabled:opacity-30">
